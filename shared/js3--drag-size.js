...
function enableDS(el, opts) {
  //enable Dragging & Sizing
  //
  // disabling / blocking
  //
  opts = opts || {};
  ...
  if (opts.block) {
    ...
    return;
  } //Functionality below not applicable to blocking element.
  //
  // initialization
  //
  'mar' in opts || (opts.mar = 25); //width of resize margin
  'movable' in opts || (opts.movable = true);
  'cb' in opts || (opts.cb = function () {});
  ...
  function findFirstThatWorks() {
    ...
    for (var i = 0; i < arguments.length - 1; ++i) {
      ...
      if (tmp.style.cursor) return arguments[i];
    } // Found one that works.
    return arguments[i];
  } // None work; return last.
  //
  // hover phase
  //
  ...
  function updateCursor(ev) {
    ...
    if (x > el.clientWidth || y > el.clientHeight) {
      //over scrollbars
      assignCursor(null);return;
    } else if (x <= opts.mar) {
      //in l margin; order of edge indices: t r b l
      opts.sizeEdges[3] && (csr = 'w-resize');
      if (y <= opts.mar) {
        //in tl corner; order of corner indices: tl tr br bl
        opts.sizeCorners[0] && (csr = 'nw-resize');
      } else if (y >= el.clientHeight - opts.mar) {
        //in bl corner
        opts.sizeCorners[3] && (csr = 'sw-resize');
      }
    } else if (x < el.clientWidth - opts.mar) {
      //between l & r margins
      if (y <= opts.mar) {
        //top edge
  ...
  function assignCursor(ev, csr) {
    ...
    if (csr === grabbing || csr === 'inherit') {
      ...
      while (par !== doc) {
        par.style.cursor = csr;par = par.parentNode;
      }
    }
  }
  //
  ...
  function moveOrResize_m(ev) {
    ...
    switch (csrStart) {
      ...
      case 'e-resize':
        moveREdge();break;
      case 'nw-resize':
        moveTEdge();moveLEdge();break;
      case 'ne-resize':
        moveTEdge();moveREdge();break;
      case 'sw-resize':
        moveBEdge();moveLEdge();break;
      case 'se-resize':
        moveBEdge();moveREdge();break;
      case grab:
  ...
  function stopMoveOrResize_m(ev) {
    ...
    if (doc.elementFromPoint(ev.clientX, ev.clientY) === efpStart) //See note above about efpStart.
      assignCursor(null, csrStart);else assignCursor(null);
  }
