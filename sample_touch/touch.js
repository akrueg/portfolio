...
chkTextLog.onclick = function () {
  return dTextLog.style.display = chkTextLog.checked ? '' : 'none';
};
sliFS.oninput = function () {
  return dTextLog.style.fontSize = sliFS.value + 'em';
};
...
chkNonWhite.onclick = function () {
  return html.style.background = chkNonWhite.checked ? 'red' : '';
};
...
function enableEventLogging() {
  var last = initialLastObj(),
      closure = initialClosureObj();
  eventsToLog().forEach(listenForIt);
  function listenForIt(type) {
    dTouch.addEventListener(type, logIt);
  }
  function logIt(ev) {
    ev.closure = closure;ev.last = last;evalAndLog(ev);
  }
}

function eventsToLog() {
  // mouse        pointer              touch
  return ['mouseover', 'pointerover', 'mouseenter', 'pointerenter', 'mouseout', 'pointerout', 'mouseleave', 'pointerleave', 'mousedown', 'pointerdown', 'touchstart', 'mousemove', 'pointermove', 'touchmove', 'mouseup', 'pointerup', 'touchend', 'pointercancel', 'touchcancel', 'click', 'gotpointercapture', 'dblclick', 'lostpointercapture', 'pointerlockchange', 'pointerlockerror'];
}
...
function initialLastObj() {
  return { t: 0, // timeStamp
    ...
    yStop: [] };
} //   so filter can gage when significant movement has resumed.
...
function initialClosureObj() {
  ...
  var colors = {};
  //major mouse events:
  colors['mousedown'] = colors['mousemove'] = colors['mouseup'] = colors['click'] = colors['dblclick'] = 'darkviolet';
  //minor mouse events:
  colors['mouseenter'] = colors['mouseleave'] = colors['mouseover'] = colors['mouseout'] = 'grey';
...
function evalAndLogTouch(ev) {
  ...
  if (chkGestures.checked && ev.type === 'touchmove' && ev.last.type === 'touchmove') if (ev.touches.length === 1) eval1TouchGesture(ev);else if (ev.targetTouches.length === 2) eval2TouchGesture(ev);
  ...
  for (var i = 0; i < ev.changedTouches.length; ++i) {
    ...
    } //Continue to next for-iteration, skipping logging.
    //Else log:
    logEv(ev);
  }
}
...
function logEv(ev) {
  //event, changedTouches index
  beginNewSequenceIfAppropriate(ev);
  //text log:
  if (chkTextLog.checked) {
    switch (ev.type) {
      case 'mousemove':
        printMouseMove(ev);break;
      case 'touchstart':
        addCoords(ev, printOtherEvent(ev));break;
      case 'touchmove':
        printTouchMove(ev);break;
      default:
        printOtherEvent(ev);}
  }
  //markers:
  switch (ev.type) {
    case 'touchstart':
      addMarker(ev);break;
    case 'touchmove':
      setMarkerPos(ev, ev.last.marker[ev.id]);}
  //cleanup:
  switch (ev.type) {
    case 'touchcancel':
    ...
    case 'touchend':
      ...
      for (var prop in ev.last) {
        if (prop !== 'marker' && Array.isArray(ev.last[prop])) ev.last[prop][ev.id] = undefined;
      }}
}
...
function beginNewSequenceIfAppropriate(ev) {
  ...
  //let the mouseout/mouseleave remain as end of old event-sequence:
    ...
    //See note above:
      ...
      //Before 2nd click NOT forming double-click:
      else if (ev.last.type === 'click' && ev.type === 'mousedown' && ev.t - ev.last.t > 333) newSequenceReason = 4;
        ...
        //Before 2nd touch NOT forming double-click:
        else if (ev.last.type === 'click' && ev.type === 'touchstart' && ev.t - ev.last.t > 300) newSequenceReason = 5;
          ...
          //After click, before moving mouse:
            ...
            else if (ev.last.type === 'touchend' && ev.last.n === 0 && ev.type === 'touchstart') newSequenceReason = 7;
  ...
  try {
    for (var _iterator = ev.last.marker[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var marker = _step.value;
      marker && dTouch.removeChild(marker);
    }
  } catch (err) {
...
function printMouseMove(ev) {
  ...
  addCoords(ev, d);
} // mousemove 134-245ms / coordinates
...
function addCoords(ev, div) {
  ...
  div.innerHTML += '<div class="coords">' + '<p>' + bullet + 'x/y: ' + Math.round(obj.pageX) + ',' + Math.round(obj.pageY) + '</p>' + '<p>' + bullet + 'cli: ' + Math.round(obj.clientX) + ',' + Math.round(obj.clientY) + '</p>' + '<p>' + bullet + 'scr: ' + Math.round(obj.screenX) + ',' + Math.round(obj.screenY) + '</p></div>';
}
...
function printOtherEvent(ev) {
  ...
  if (ev.type === 'click' && dTextLog.children.length >= 3) {
    var d1 = dTextLog.children[dTextLog.children.length - 3],
        ...
        s1 = d1.innerHTML.split(' '),
        //eg ["mmove","33-33ms"]
    s2 = d2.innerHTML.split(' '),
        //eg ["mdown","33ms"   ]
    s3 = d3.innerHTML.split(' '); //eg ["mup"  ,"33ms"   ]
    if (s1[0] === 'mmove' && s2[0] === 'mdown' && s3[0] === 'mup' && s2[1] === s3[1]) {
      //abbreviate:
...
function saveEventInfo(ev) {
  //Record info for next event's use:
  ev.last.t = ev.t;
...
function timeStamp(ev) {
  if (ev.closure.reliableTimeStamp === undefined) ev.closure.reliableTimeStamp = ev.timeStamp.toString().match(/\./) ? true : false;
...
function eval1TouchGesture(ev) {
  var tch = ev.touches[0],
      id = tch.identifier,
      dX = tch.pageX - ev.last.x[id],
      dY = tch.pageY - ev.last.y[id],
      dT = ev.t - ev.last.t,
      vX = dX / dT,
      vY = dY / dT,
      aVX = Math.abs(vX),
  ...
  if (vX > thresh) assignGesture(ev, 'swipedRight');else if (vX < -thresh) assignGesture(ev, 'swipedLeft');else if (vY > thresh) assignGesture(ev, 'swipedDown');else if (vY < -thresh) assignGesture(ev, 'swipedUp');
}

function eval2TouchGesture(ev) {
  if (ev.last.n !== 2) return; //Make sure last event was also 2-touch.
  var tch1 = ev.touches[0],
      tch2 = ev.touches[1],
      id1 = tch1.identifier,
      id2 = tch2.identifier,

  //box formed by 2 fingers:
  wOld = ev.last.x[id1] - ev.last.x[id2],
      //width
  hOld = ev.last.y[id1] - ev.last.y[id2],
      //height
  dOld = hypotenuse(wOld, hOld),
      //diagonal / distance
  wNew = tch1.pageX - tch2.pageX,
      ...
      vD = dD / dT,
      thresh = .1;
  if (vD > thresh) assignGesture(ev, 'spread');else if (vD < -thresh) assignGesture(ev, 'pinched');
}
...
function significanceOf(ev) {
  ...
  if (ev.last.tMovement[ev.id] === undefined) {
    //1st movement
  ...
  var dP = hypotenuse(dX, dY),
      v = dP / dT;
  if (ev.last.xStop[ev.id] === undefined) {
    //in motion
    if (v < minVelocity) {
      //this motion signals stopping
    ...
    return 4;
      ...
      //stopped
      var tmdStyle = ev.last.tmoveDiv[ev.id].style,
          //for toggling bold, to show filtered motion
      ...
      if (dFromStop < minRadius) {
        //stopped, not past threshold
...
} //was stopped, has now passed threshold
...
function enableRawLogging() {
  ...
  function hookItUp(evName) {
    win.addEventListener(evName, logRaw, true);
  }
  function logRaw(ev) {
    log(idOf(ev.target) + '.' + ev.type);
  }
}
