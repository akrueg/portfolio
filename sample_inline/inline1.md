...
A block box with text generates a 'block formatting context' and contains 
one block-width ____line box____ per line of inline text, without vertical 
overlapping or gaps.  The block below contains 2 light-blue line boxes 
with no visible boundary between them: 
...
it contains 2 (green) line boxes, while the surrounding `<p>` contains 
only 1 (blue) line box.

Visualizing the individual line boxes is a challenge.  There's no obvious 
way to see that the line boxes above have unequal heights due to the `<span>`s' 
...
are ____content boxes____ (green) and do not affect line box height; each inline 
element also generates an unstylable (sans hack) ____inline box____ (pink) that 
