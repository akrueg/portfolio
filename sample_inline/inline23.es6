...
function onclickNormal(){
  setStyles('line-height',
            this.checked? 'normal' : Ctr01rngLH.value); calc() }
function onclickAuto(){
  setStyles('height',
            this.checked? 'auto' : Ctr01rngH.value+'px'); calc() }
...
function rngOnkeydown(ev){
  ...
  switch(ev.keyCode){
    ...
    case 37: this.value-=extra; break
    case 39: this.value=parseFloat(this.value)+extra; break
    case 38: case 40: ev.preventDefault()
      ;(this===Ctr01rngLH ? Ctr01rngH : Ctr01rngLH).focus() }}

//Controls contained in following analysis:
Ctr01chkMarker.onclick=()=>Ex01s4.innerHTML=Ctr01chkMarker.checked?'Aib':'ib'
Ctr01btn1.onclick=()=>btnSetStyles(.3,'auto')
Ctr01btn2.onclick=()=>btnSetStyles(2.5,50)
Ctr01btn3.onclick=()=>btnSetStyles(0,0)
Ctr01btn4.onclick=()=>btnSetStyles(0,40)
Ctr01btn5.onclick=()=>btnSetStyles(0,60)
function btnSetStyles(lh,h){
  if (lh==='normal') {Ctr01chkNormal.checked=true; Ctr01chkNormal.onclick()}
  else {              Ctr01chkNormal.checked=false
                      Ctr01rngLH    .value  =lh  ; Ctr01rngLH    .oninput() }
  if (h==='auto') {   Ctr01chkAuto  .checked=true; Ctr01chkAuto  .onclick() }
  else {              Ctr01chkAuto  .checked=false
                      Ctr01rngH     .value  =h   ; Ctr01rngH     .oninput() }}
Ctr01chkUnwrapped.onclick=()=>{
...
Ctr02chk1 .onclick=()=> 
  setEx02(Ctr02chk1.checked? 'auto' : Ctr02rngH.value+'px')
Ctr02rngLH.oninput=()=>setEx02(null, Ctr02rngLH.value)
Ctr02rngH .oninput=()=>
  {setEx02(Ctr02rngH.value+'px'); Ctr02chk1.checked=false}
function setEx02(h, lh){
  ;[Ex02s1,Ex02s2,Ex02s3,Ex02img1].forEach(el=>{
     h&&(el.style.    height= h)
...
// example 4
Ctr04txt1.oninput=()=>Ex04s.innerHTML=Ctr04txt1.value
Ctr04chk1.onclick=()=>
  Array.from(document.querySelectorAll('#Ex04 span:not([id])')).forEach(
    el=>el.style.verticalAlign=Ctr04chk1.checked?'bottom':'')
Ctr04chk2.onclick=()=>{
