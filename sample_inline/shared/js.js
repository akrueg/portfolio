...
function bindSelect(sel, el, style, useLabels) {
  sel.onchange = function () {
    return el.style[style] = useLabels ? labelOf(sel) : sel.value;
  };
}
...
function bindText(txt, el, style) {
  txt.oninput = function () {
    return el.style[style] = txt.value;
  };
}
function bindCheck(chk, el, style, tval, fval) {
  chk.onclick = function () {
    return el.style[style] = chk.checked ? tval : fval;
  };
}
...
function formatFormulae() {
  ...
  try {
    for (var _iterator = qsa('.formula, .formulaMain, .formulaTable')[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var el = _step.value;
...
function inlineFormattingInit() {
  ...
  p.id = 'pDisclaimer';
  p.innerHTML = 'The observations below are based on the behavior of Chromium / ' + 'Chrome browser.  The Spec leaves several implementation details to ' + 'the browser, including the definition of {line-height: normal}, ' + 'which affects most inline formatting calculations.';
...
function addNav() {
  ...
  // Link formats:
  //   GitHub:  https://akrueg.github.io/portfolio/sample_inline/inline1.html
  ...
  // Previous:
  if (i > 0) {
    addChild(pNav, link('Previous', base + 'sample_inline/inline' + ppGitHub[i - 1]));
    // addText (pNav,bullet) 
  ...
  if (i < ppGitHub.length - 1) {
    addChild(pNav, bullet());
    // addText (pNav,bullet)
    addChild(pNav, link('Next', base + 'sample_inline/inline' + ppGitHub[i + 1]));
  }
